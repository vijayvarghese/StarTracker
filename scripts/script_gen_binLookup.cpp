...
void loadBin(const std::string& filename, std::unordered_map<int, std::vector<StarPair>>& lookup) {
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        std::cerr << "Usage: Bin_Gen <input.csv> <output.bin> <fov_degrees>" << std::endl;
        return -1;
    }
    ...
    std::string line;
    if (!std::getline(file, line)) return 0; // skip header
    ...
    std::cout << "CSV load: " << std::chrono::duration<double>(t2 - t1).count() << " sec" << std::endl;
    std::cout << "Total pairs: " << total << " | Skipped (> " << fov << "°): " << skipped
              << " | Kept: " << (total - skipped) << std::endl;
    ...
    std::cout << "Binary load: " << std::chrono::duration<double>(t2 - t1).count() << " sec" << std::endl;
    ...
    if (it == lookup.end())
        ...
        std::cout << "Spot check key 995: " << it->second[0].id1 << " , " << it->second[0].id2 << std::endl;
