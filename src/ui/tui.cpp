...
void TuiThread::start() { thread_ = std::thread(&TuiThread::run, this); }
...
void TuiThread::run() {
  ...
  auto ui = Renderer([&] {
    // ── title ─────────────────────────────────────────────────────────
    auto title = text(" " + name_ + "  v" + version_ + " ") | bold |
                 color(Color::RGB(100, 200, 255)) | flex;
    ...
    auto log_cell = hbox({
                        ...
                        text(std::string(log_labels[log_idx]) + " ") | bold |
                            color(log_color(log_idx)),
                    }) |
                    border;
