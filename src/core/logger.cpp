...
// Using initializer list to create object with parameter initialized (Faster
// than creating object then assigning param)
...
LogMessage::~LogMessage() {
  ...
  case 1:
    std::cerr << "[" << level_to_string(level_) << "] " << stream_.str()
              << "\n";
    break;

  case 2:
    std::cerr << "[" << ms << "] "
              << "[" << level_to_string(level_) << "] " << stream_.str()
              << "\n";
    break;
