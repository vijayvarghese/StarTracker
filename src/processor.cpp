...
#define PRINT_ANGULAR_SEP_PROFILE(profile_vec)                                 \
  do {                                                                         \
    LOG_DEBUG << "=== AngularSeparationProfile [" << (profile_vec).size()      \
              << " profiles] ===";                                             \
    for (size_t _pi = 0; _pi < (profile_vec).size(); _pi++) {                  \
      ...
      LOG_DEBUG << "  │  angle       : " << std::fixed << std::setprecision(4) \
                << _p.centroid_ray_profile.angular_separation << " deg";       \
                                                                               \
      LOG_DEBUG << "  │  centroids   : (" << std::fixed                        \
                << std::setprecision(2)                                        \
                << _p.centroid_ray_profile.centeroid_pair[0].x << ", "         \
                ...
                << _p.centroid_ray_profile.centeroid_pair[1].y << ")";         \
                                                                               \
      LOG_DEBUG << "  │  rays        : [" << std::setprecision(5)              \
                << _p.centroid_ray_profile.ray_pair[0][0] << ", "              \
      ...
      if (_p.candidate_pair.empty()) {                                         \
        ...
        LOG_DEBUG << "  │  candidates  : " << _p.candidate_pair.size()         \
                  << " pairs";                                                 \
      }                                                                        \
      LOG_DEBUG << "  └─────────────────────────────────";                     \
    }                                                                          \
...
void processor_thread(std::atomic<std::shared_ptr<cv::Mat>> &latest_frame,
  ...
  while (running.load()) {
    ...
    preprocess_ed_frame = ST::image::preprocess_frame(
        frame_cpy_local, tsec, processor_cfg.cv.blur_ksize,
        processor_cfg.cv.blur_sigma,
        processor_cfg.cv.threshold); // processing level -0
    centroids = ST::image::get_centroids(preprocess_ed_frame, tsec,
    ...
    for (const auto &c : centroids) {
      ...
      temp_ray = ST::math::pixel_to_body_ray(
          c.x, c.y, camera_intr_cfg.fx, camera_intr_cfg.fy, camera_intr_cfg.cx,
          camera_intr_cfg.cy);
      if (ST::dbg::ray) {
        LOG_DEBUG << "[Tracker] [Centroid] " << "(" << c.x << ", " << c.y
                  << ")";
        LOG_DEBUG << "[Tracker] [Centroid] [Ray] = (" << temp_ray[0] << ", "
                  << temp_ray[1] << ", " << temp_ray[2] << ")";
      }
    ...
    // LOG_DEBUG << "[Tracker] [Centroid] [Ray] = ("<< ray[0] << "), (" <<
    // ray[1] << "), Angular separation -> " <<angle_between(ray[0],ray[1])<<
    // "";
    ...
    for (size_t i = 0; i < pair.ray_pair.size(); i++) {
      for (size_t j = i + 1; j < pair.ray_pair.size(); j++) {
        ...
        double theta =
            ST::math::AngularSeparationDeg(pair.ray_pair[i], pair.ray_pair[j]);
        ...
        if (ST::dbg::ang_sep) {
          LOG_DEBUG << "[Tracker] [Centroid] [Ray] [Ang_Sep][" << i << "," << j
                    << "]= (" << pair.ray_pair[i] << "), (" << pair.ray_pair[j]
                    << "), Angular separation -> " << theta << "";
        }
      }
    }

    ST::identification::nStar::PopulateCandidatePairs(
        AngularSeparationProfile, lookup, lookup_cfg.tolerance,
        lookup_cfg.lookup_precision);
    if (ST::dbg::ang_profile) {
      PRINT_ANGULAR_SEP_PROFILE(AngularSeparationProfile);
    }

    hypothesis = ST::identification::nStar::vote_and_hypothesize(
    ...
    tsec += std::chrono::duration_cast<std::chrono::duration<double>>(
                processor_cfg.period)
                .count();
    // LOG_DEBUG<<"Processor Frame : "<<frame_cpy_local.cols<<" x
    // "<<frame_cpy_local.rows<<"";
  }
