...
bool PopulateCandidatePairs(
  ...
  for (auto &profile : profiles) {
    int center_bin = (int)std::round(
        profile.centroid_ray_profile.angular_separation / precision);
    for (int offset = -tolerance; offset <= tolerance; ++offset) {
      ...
      if (it != lookup_.end()) {
        ...
        profile.candidate_pair.insert(profile.candidate_pair.end(), can.begin(),
                                      can.end());
      }
    }
  }
                     ...
                     int N) // number of detected centroids — 9 in your case
{
  ...
  std::map<cv::Point2d, std::unordered_map<std::string, int>, Point2dCmp>
      star_votes;
  ...
  for (const auto &prof : profiles) {
    ...
    for (const auto &id : seen_ci)
      star_votes[ci][id]++;
    for (const auto &id : seen_cj)
      star_votes[cj][id]++;
  }
  // debug
  {
  ...
  for (auto &[pixel, id_map] : star_votes) {
    ...
    h.hip_id_second_best =
        second_id; // for later debug, remove if not implimented !! along with
                   // the second_id ref on top.
